...
async def channel_post(client: Client, message: Message):
    ...
    try:
        ...
        await reply_text.edit_text("<b>Wait a minuter...</b>")
        return
        ...
        [
            [
                InlineKeyboardButton(
                    "🔁 Share Link", url=f"https://t.me/{client.username}?start={base64_string}"
                )
            ]
        ]
    )
...
async def new_post(client: Client, message: Message):
    ...
    reply_markup = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "🔁 enlaces rápidos", url=f"https://t.me/{client.username}?start={base64_string}"
                )
            ]
        ]
    )
